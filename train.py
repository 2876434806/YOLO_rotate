...
from models.yolo import Model, BiFPN_Add2, BiFPN_Add3, BiFPN_Concat2, BiFPN_Concat3
...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    imgsz = check_img_size(opt.imgsz, gs, floor=gs * 2)  # verify imgsz is gs-multiple// 看是否能被最大下采样倍数整除
    ...
    hyp['cls'] *= nc / 80 * 3 / nl  # scale to classes and layers # 80是否可以改变
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            #
            #     writer.add_image('data/Image', image, iter_num)
            #     outputs = torch.argmax(torch.softmax(outputs, dim=1), dim=1, keepdim=True)
            #     writer.add_image('data/Prediction', outputs[1, ...] * 50, iter_num)
            #     labs = label_batch[1, ...].unsqueeze(0) * 50
            #     writer.add_image('data/GroundTruth', labs, iter_num)
        ...
        if RANK in [-1, 0]:
            ...
            if not noval and epoch >= opt.begin_val or final_epoch:  # Calculate mAP
...
def parse_opt(known=False):
    ...
    parser.add_argument('--cfg', type=str, default=ROOT /'C:\Code\yolov5_rotation-master\models\c3_ghost_mutil_sk_add.yaml', help='model.yaml path')
    parser.add_argument('--data', type=str, default=ROOT / 'data/hrsc.yaml', help='dataset.yaml path')
    ...
    parser.add_argument('--adam', default=True, help='use torch.optim.Adam() optimizer')
    ...
    parser.add_argument('--begin_val', type=int, default=0, help='When will validation begin, default No.30 epoch')
