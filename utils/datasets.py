    ...
    def __getitem__(self, index):
        ...
        if self.augment:
            ...
            if nl:
                ...
                #np.savetxt('angle_class\\'+ str(index) +'.txt', labels, delimiter=' ')
...
def load_mosaic(self, index):
    ...
    for i, index in enumerate(indices):
        ...
        img, _, (h, w) = load_image(self, index)  # (h, w)  hw_resized

        # place img in img4
        if i == 0:  # top left
            img4 = np.full((s * 2, s * 2, img.shape[2]), 114, dtype=np.uint8)  # base image with 4 tiles
            x1a, y1a, x2a, y2a = max(xc - w, 0), max(yc - h,
                                                     0), xc, yc  # xmin, ymin, xmax, ymax (large image)  用于确定原图片在img4左上角的坐标（左上右下）
            x1b, y1b, x2b, y2b = w - (x2a - x1a), h - (
                        y2a - y1a), w, h  # xmin, ymin, xmax, ymax (small image)  用于确定原图片剪裁进img4中的图像内容范围
        elif i == 1:  # top right
            x1a, y1a, x2a, y2a = xc, max(yc - h, 0), min(xc + w, s * 2), yc
            x1b, y1b, x2b, y2b = 0, h - (y2a - y1a), min(w, x2a - x1a), h
        elif i == 2:  # bottom left
        ...
        if x.size > 0:  # Normalized xywh to pixel xyxy format 归一化的xywh转为非归一化的xyxy（左上右下）坐标形式
            ...
            labels[:, 1] = max_w_h * (x[:, 1] - x[:, 3] / 2) + padw  # Left_top_x
            labels[:, 2] = max_w_h * (x[:, 2] - x[:, 4] / 2) + padh  # Left_top_y
            labels[:, 3] = max_w_h * (x[:, 1] + x[:, 3] / 2) + padw  # right_bottom_x
            labels[:, 4] = max_w_h * (x[:, 2] + x[:, 4] / 2) + padh  # right_bottom_y
        labels4.append(
            labels)  # labels4：[array([对应1.txt的labels信息 size=[n1,6]], ... ,array([对应4.txt的labels信息] size=[n4,6]]
...
def verify_image_label(args):
    ...
    try:
        ...
        if os.path.isfile(lb_file):
            ...
            nl = len(l)
            if nl:
                assert l.shape[1] == 6, f'labels require 6 columns for rotation, {l.shape[1]} columns detected'
                assert (l >= 0).all(), f'negative label values {l[l < 0]}'
                # assert (l[:, 1:5] <= 1).all(), f'non-normalized or out of bounds coordinates {l[:, 1:][l[:, 1:] > 1]}'
