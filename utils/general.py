...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.2, classes=None, agnostic=False, multi_label=False,
                        labels=(), max_det=300):
    ...
    nc = prediction.shape[2] - 5 - 8  # number of classes
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if labels and len(labels[xi]):
            l = labels[xi]
            #v = torch.zeros((len(l), nc + 5 + 180), device=x.device)
            v = torch.zeros((len(l), nc + 5 + 8), device=x.device)
            v[:, :4] = l[:, 1:5]  # box
            v[:, 4] = 1.0  # conf
            v[range(len(l)), l[:, 0].long() + 5] = 1.0  # cls
            v[range(len(l)), l[:, 5].long() + 5 + nc] = 1.0  # angle
        ...
        if multi_label:
            i, j = (x[:, 5:5 + nc] > conf_thres).nonzero(as_tuple=False).T
            pre_origin = x[i, 5 + nc:]
            pre_origin = pre_origin.cpu().detach().numpy()
            pre_origin = np.where(pre_origin < 0.5, 0, 1)  # 根据门限值将预测值改为二进制码0或1
            pre_decimal = angle_label_decode(pre_origin, 180, omega=180 / 256., mode=1)  # pre 2 -> 10
            pre_decimal = torch.tensor(pre_decimal)
            pre_decimal = pre_decimal.to(device)
            pre_decimal = pre_decimal.reshape(-1, 1)
            # conf_angle, j_angle = x[i, 5 + nc:].max(1, keepdim=True)
            x = torch.cat((box[i], x[i, j + 5, None], j[:, None].float(), pre_decimal.float()), 1)
            xy = xy[i]
            wh = wh[i]
            #     if multi_label:
            #         i, j = (x[:, 5:5 + nc] > conf_thres).nonzero(as_tuple=False).T
            ...
            conf, j = x[:, 5:5 + nc].max(1, keepdim=True)
            # conf_angle, j_angle = x[:, 5 + nc:].max(1, keepdim=True) #CSL
            inds = conf.view(-1) > conf_thres

            # a = x[:, 5 + nc:].cpu().detach().numpy()
            #
            # t = ''
            # with open('angle_origin.txt', 'w') as q:
            #     for i in a:
            ...
            # pre_origin = torch.sigmoid(x[:, 5 + nc:])
            pre_origin = x[:, 5 + nc:]
            pre_origin = pre_origin.cpu().detach().numpy()
            pre_origin = np.where(pre_origin < 0.5, 0, 1)  # 根据门限值将预测值改为二进制码0或1
            pre_decimal = angle_label_decode(pre_origin, 180, omega=180 / 256., mode=1)  # pre 2 -> 10
            ...
            # x = torch.cat((box, conf, j.float(), j_angle.float()), 1)[inds] #CSL
            x = torch.cat((box, conf, j.float(), pre_decimal.float()), 1)[inds]
            xy = xy[inds]
            wh = wh[inds]
