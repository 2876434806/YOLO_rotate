...
from utils.metrics import ConfusionMatrix, ap_per_class, rotate_box_iou
...
def process_batch(detections, labels, iouv):
    ...
    if x[0].shape[0]:
        ...
        matches = torch.Tensor(matches).cuda()
...
def run(data,
    ...
    if not training:
        ...
        dataloader = create_dataloader(data[task], imgsz, batch_size, stride, single_cls,rect=pt,
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        # NMS
        targets[:, 2:6] *= torch.Tensor([width, height, width, height]).cuda()  # to pixels
    ...
    # Print results
    pf = '%20s' + '%11i' * 2 + '%11.5g' * 4  # print format
...
def parse_opt():
    ...
    # parser.add_argument('--weights', nargs='+', type=str, default= 'F:\Code\部分训练结果存档\\6.6日之后\\train\exp37\weights\\best.pt', help='model.pt path(s)')
    parser.add_argument('--weights', nargs='+', type=str, default='F:\Code\hrsc存档\\train\exp43\weights\\best.pt', help='model.pt path(s)')
    parser.add_argument('--batch-size', type=int, default=32, help='batch size')
    parser.add_argument('--imgsz', '--img', '--img-size', type=int, default=800, help='inference size (pixels)')
    parser.add_argument('--conf-thres', type=float, default=0.4, help='confidence threshold')#0.4
    parser.add_argument('--iou-thres', type=float, default=0.2, help='NMS IoU threshold')#0.2
